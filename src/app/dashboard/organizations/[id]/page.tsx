...
export default function ProjectsPage() {
  ...
  const orgId = searchParams.id as string;
  const { user } = useAuthStore();
  const canManageProjects = user?.role === 'ADMIN' || user?.role === 'SUB_ADMIN';
  const [projects, setProjects] = useState<Project[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [newProject, setNewProject] = useState({ name: '', description: '', organizationId: orgId });
  ...
  const fetchProjects = async () => {
    try {
      if (!orgId) return;
      const { data } = await api.get(`/projects?orgId=${orgId}`);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setIsCreating(true);
    try {
      await api.post('/projects', { ...newProject, organizationId: orgId });
      setShowCreateModal(false);
      setNewProject({ name: '', description: '', organizationId: orgId });
      fetchProjects();
    } catch (e) {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => {
          const hasAccess = project.isJoined || canManageProjects;
          const content = (
            <div className={`bg-card border border-border p-6 rounded-3xl relative overflow-visible transition-all ${
              hasAccess ? 'hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 cursor-pointer' : 'opacity-80'
            }`}>
              ...
              <div className="absolute top-4 right-4 flex items-center gap-1">
                ...
                {hasAccess ? (
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-secondary text-muted-foreground text-[10px] font-bold border border-border uppercase tracking-wide">
                    Not Joined
                  </span>
                )}
              </div>
          ...
          return (
            <div key={project.id} className="group relative">
              {hasAccess ? (
                ...
                <div title="You are not a member of this project yet">
                  {content}
                </div>
              )}
            </div>
